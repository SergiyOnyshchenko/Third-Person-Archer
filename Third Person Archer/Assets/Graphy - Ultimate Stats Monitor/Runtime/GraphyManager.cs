...
namespace Tayx.Graphy
{
    ...
    public class GraphyManager : G_Singleton<GraphyManager>
    {
        ...
        {
            System.Collections.IEnumerator SimulateFrameRate()
            {
                float initialDropDuration = 5.0f;
                float unstableDuration = 2.0f;
                float recoveryDuration = 2.0f;
                // Первая фаза: плавный дроп FPS до 1
                float elapsed = 0f;
                while (elapsed < initialDropDuration)
                {
                    elapsed += Time.deltaTime * 10;
                    Application.targetFrameRate = (int)Mathf.Lerp(60, 1, elapsed / initialDropDuration);
                    yield return null;
                }

                // Вторая фаза: нестабильный FPS между 10 и 20
                elapsed = 0f;
                while (elapsed < unstableDuration)
                {
                    elapsed += 0.1f;
                    Application.targetFrameRate = UnityEngine.Random.Range(10, 20);
                    yield return new WaitForSecondsRealtime(0.1f); // изменяем FPS каждые 0.1 секунды
                }

                // Третья фаза: восстановление до нормального FPS
                elapsed = 0f;
                while (elapsed < recoveryDuration)
                {
                    elapsed += Time.deltaTime;
                    Application.targetFrameRate = (int)Mathf.Lerp(20, 60, elapsed / recoveryDuration);
                    yield return null;
                }

                // Возвращаемся к нормальному FPS
                Application.targetFrameRate = 60;
            }

            StartCoroutine(SimulateFrameRate());
        }
