        ...
        public class YCGameutilsUpdateWindow : YCCustomWindow {
            ...
            private void OnGUI() {
                ...
                if (this.data.isUpToDate) {
                    ...
                    YCEditorGUILayout.AddLabel("GameUtils is already up to date.", TextAnchor.MiddleCenter, style);
                    ok = "Reimport";
                } else {
                    style.normal.textColor = Color.red;
                    YCEditorGUILayout.AddLabel("GameUtils needs to be updated.", TextAnchor.MiddleCenter, style);
                    ok = "Update";
                }
                YCEditorGUILayout.AddEmptyLine();
                YCEditorGUILayout.AddLabel("Do you want to import the version " + this.data.version + "?");
                this.useAdvancedSettings = YCEditorGUILayout.AddToggle("Advanced settings", this.useAdvancedSettings);
                this.data.isAdvanced = this.useAdvancedSettings;
                if (this.useAdvancedSettings) {
                    this.deleteObsoletes = YCEditorGUILayout.AddToggle("Delete obsolete files and folders before importing (recommended)", this.deleteObsoletes);
