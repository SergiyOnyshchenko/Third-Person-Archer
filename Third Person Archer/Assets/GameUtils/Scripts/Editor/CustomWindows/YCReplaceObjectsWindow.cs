...
namespace YsoCorp {
    namespace GameUtils {
        ...
        public class YCReplaceObjectsWindow : YCCustomWindow {
            ...
            private void OnGUI() {
                YCEditorGUILayout.AddLabel("Replace all currently selected GameObjects with the one below.\n");
                gameObjectModelReplacing = YCEditorGUILayout.AddObjectField<GameObject>("New GameObject", gameObjectModelReplacing, true);
                ...
                this._useScale = YCEditorGUILayout.AddToggle("Override Scale", this._useScale);
                this._useName = YCEditorGUILayout.AddToggle("Override Name", this._useName);

                GUI.enabled = gameObjectModelReplacing != null && gameObjectsToReplace.Count > 0;
                YCEditorGUILayout.AddButton("Replace", () => {
