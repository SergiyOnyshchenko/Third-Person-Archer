...
{
    public class ThrowableWeaponFpvSkinView : Actor.System
    {
        [Serializable]
        public sealed class Entry
        {
            ...
            [Tooltip("Root object of this FPV variant (the model to show)")]
        ...
        protected virtual void Awake()
        {
            RebuildMap();
            HideAll();
            if (!string.IsNullOrEmpty(_defaultWeaponId))
                SetView(_defaultWeaponId);
        }
        ...
        protected virtual void OnValidate()
        {
            ...
            foreach (var e in _entries)
            {
                ...
                if (!seen.Add(e.WeaponId))
                    Debug.LogWarning($"{LogPrefix}: duplicate WeaponId '{e.WeaponId}' on {name}. First entry will be used.");
            }
        }
        ...
        public bool SetView(string weaponId)
        {
            ...
            {
                Debug.LogWarning($"{LogPrefix}.SetView called with null/empty id.");
                return false;
            }
            ...
            {
                Debug.LogWarning($"{LogPrefix}: no entry for id '{weaponId}' on {name}.");
                return false;
            }
        ...
        /// <summary>Rebuild internal lookup map from the serialized list.</summary>
        ...
        /// <summary>Deactivate all variant roots.</summary>
